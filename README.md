...
- **内置适配器** - 提供多种内置适配器，包括控制台、回显和测试适配器
- **丰富事件类型** - 支持文本、图片、语音、通知等多种事件类型
- **统计监控** - 内置适配器统计系统，实时监控事件处理情况
...
框架支持丰富的事件类型，涵盖即时通讯的各种场景：

#### 消息事件（MessageEvent）
- **Text** - 文本消息，包含文本内容和元数据
- **Image** - 图片消息，包含图片URL和可选的图片说明
- **Voice** - 语音消息，包含语音文件URL和时长
- **Video** - 视频消息（预留）
- **File** - 文件消息（预留）

#### 通知事件（NoticeEvent）
- **SystemNotice** - 系统通知
- **FriendRequest** - 好友请求（预留）
- **GroupInvite** - 群组邀请（预留）
- **MemberChange** - 成员变更（预留）

#### 事件元数据（EventMetadata）
每个事件都包含丰富的元数据信息：
- `source` - 事件来源（用户、系统、定时器等）
- `user_id` - 用户ID
- `group_id` - 群组ID（可选）
- `timestamp` - 时间戳
- `event_id` - 事件唯一标识符
...
- **定时生成测试事件**，默认每5秒生成一个事件
- **轮换事件类型**：文本消息、图片消息、语音消息、系统通知、群组消息
- **统计监控**：实时跟踪事件生成数量和错误情况
- **可配置间隔**：通过 `event_interval_seconds` 配置项调整事件生成频率
- **适用场景**：
  - 测试框架完整功能（事件处理、统计、热重载等）
  - 压力测试和性能评估
  - 验证工作流各阶段的处理逻辑

**MockTestAdapter 配置示例**：
```json
{
